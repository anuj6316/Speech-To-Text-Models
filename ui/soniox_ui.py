...
class TranscriptionSession:
    def __init__(self, api_key: str, audio_format: str = "pcm_s16le", translation: str = "none"):
    ...
    def render_tokens(self, final_tokens: list[dict], non_final_tokens: list[dict]) -> str:
        text_parts = []
        ...
        try:
            while not self.stop_event.is_set():
                ...
                if res.get("error_code") is not None:
                    self.current_transcript = f"Error: {res['error_code']} - {res['error_message']}"
                    break
                ...
                self.current_transcript = self.render_tokens(self.final_tokens, non_final_tokens)
